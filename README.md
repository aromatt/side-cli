...
* `%1` refers to the stream of values generated by the coprocess. This is similar to
  `\1` in sed or `$1` in awk: the variable takes on a different value for each line
  of input. The number `N` in `%N` refers to the output of the Nth coprocess, with
  `%0` being xcopr's original stdin.
